...
# pylint: disable=bare-except
            try:
                ...
                if not msg:
                    logger.error("Receive error in UART socket")
                    self.connected.clear()
                    break
                for b in msg:
                ...
                logger.exception("Error in UART socket read thread")
    ...
    def write(self):
        ...
        while self.write_running.is_set():
            try:
                ...
                if sent != len(msg):
                    logger.error(f"Send error: {sent}/{len(msg)} msgs sent")
            except queue.Empty:
                continue
        ...
        if self.read_thd and self.read_thd.is_alive():
            ...
            if self.read_thd.is_alive():
                logger.warning("Failed to join read thread in time")
