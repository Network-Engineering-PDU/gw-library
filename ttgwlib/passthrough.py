...
# pylint: disable=bare-except
        try:
            ...
            logger.exception("Error in UART relay to socket process")
...
# pylint: disable=bare-except
            try:
                ...
                logger.exception("Error while keeping connection process")
                if self.socket:
                    self.socket.close()
                continue
            finally:
    ...
    def stop(self):
        ...
        if self.socket_thd and self.socket_thd.is_alive():
            ...
            if self.socket_thd.is_alive():
                logger.warning("Failed to join Socket thread in time")
        if self.uart:
            self.uart.stop()
